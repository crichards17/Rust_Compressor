...
describe("IdCompressor", () => {
	...
	describe("ID Generation", () => {
		itCompressor("can create a compressed ID with an override", () => {
		...
		itCompressor("can create compressed IDs with v5 overrides", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
		...
		itCompressor("will not decompress IDs it did not compress", () => {
		...
		itCompressor("will not re-compress uuids it did not originally compress", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			assert.equal(
		...
		itCompressor("unifies duplicate overrides originating from the same compressor", () => {
			const override = "override";
		...
		itCompressor("unifies overrides with sequential local IDs", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 3);
		...
		itCompressor(
			"unifies overrides with sequential local IDs that sort after an existing override",
			() => {
				...
				const compressor = CompressorFactory.createCompressorWithSession(newSession);
		...
		itCompressor(
			"unifies overrides with sequential local IDs that sort after an existing cluster",
			() => {
				...
				const compressor1 = CompressorFactory.createCompressorWithSession(newSession1);
				const compressor2 = CompressorFactory.createCompressorWithSession(newSession2);
		...
		itCompressor(
			"unifies unfinalized local overrides with final IDs from a remote session",
			() => {
				const compressor1 = CompressorFactory.createCompressor(Client.Client1, 3);
				const compressor2 = CompressorFactory.createCompressor(Client.Client2, 3);
		...
		itCompressor("unifies overrides with sequential local IDs that have been finalized", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
	...
	describe("Finalizing", () => {
		itCompressor(
			"can finalize multiple overrides into the same cluster using different ranges",
			() => {
				const compressor = CompressorFactory.createCompressor(Client.Client1);
		...
		itCompressor("prevents attempts to finalize ranges twice", () => {
			...
			const explicitCompressor = CompressorFactory.createCompressor(Client.Client1);
	...
	describe("Compression", () => {
		itCompressor("can re-compress a sequential uuid it generated", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
			const uuid = compressor.decompress(id);
		...
		itCompressor("can re-compress an override", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const override = "override";
			const id = compressor.generateCompressedId(override);
		...
		itCompressor("can re-compress overrides from a remote client it has finalized", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
			const override = "override";
		...
		itCompressor("will not compress an override it never compressed or finalized", () => {
			...
			assert.equal(
				compressor.tryRecompress(incrementStableId(sessionIds.get(Client.Client1), 1)),
				undefined,
			);
			compressor.generateCompressedId(override);
			...
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
			assert.equal(
				compressor.tryRecompress(incrementStableId(sessionIds.get(Client.Client1), 4)),
				undefined,
			);
		});

		itCompressor("can re-compress an eagerly generated final ID that is not finalized", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
			compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
	...
	describe("Decompression", () => {
		...
		itCompressor("can decompress a final ID with an override", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const override = "override";
			const id = compressor.generateCompressedId(override);
			const range = compressor.takeNextCreationRange();
		...
		itCompressor("can decompress an override that is an UUID", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
		...
		itCompressor("properly sorts UUID-like overrides separately from true UUIDs", () => {
		...
		itCompressor(
			"can decompress an override that starts with the reserved prefix character",
			() => {
				const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
	...
	describe("Telemetry", () => {
		...
		itCompressor("emits correct telemetry status with overrides", () => {
			const mockLogger = new MockLogger();
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5, mockLogger);
			const localId1 = compressor.generateCompressedId();
			assert(isLocalId(localId1));
	...
	describe("Eager final ID allocation", () => {
		...
		itCompressor("does not eagerly allocate final IDs for IDs with overrides", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
			const localId1 = compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
	...
	describe("Serialization", () => {
		...
		itCompressor("correctly deserializes and resumes a session", () => {
			...
			const compressor2 = CompressorFactory.createCompressor(Client.Client2, undefined);
			compressor1.generateCompressedId();
	...
	describeNetworkNoValidation("detects UUID collision", (itNetwork) => {
		itNetwork(
			...
			(network) => {
				...
				const nextUuid = incrementStableId(uuid, 1);
		...
		itNetwork(
			...
			(network) => {
				...
				const nextUuid = incrementStableId(uuid, 2);
				network.allocateAndSendIds(Client.Client1, 1, { 0: nextUuid });
				network.allocateAndSendIds(Client.Client2, 1);
				network.deliverOperations(DestinationClient.All);
		...
		itNetwork("detects colliding override UUIDs when expanding a cluster", 1, (network) => {
			...
			const uuid = assertIsStableId(compressor1.decompress(id));
			const expansion = 3;
			const nextUuid = incrementStableId(uuid, expansion);
