...
import { createSessionId, fail, isStableId } from "../../src/util/utilities";

describe("IdCompressor", () => {
	itCompressor("detects invalid cluster sizes", () => {
	...
	itCompressor("reports the proper session ID", () => {
	...
	describe("ID Generation", () => {
		itCompressor("can manually create a compressed ID", () => {
		...
		describe("Eager final ID allocation", () => {
			itCompressor(
				"eagerly allocates final IDs when cluster creation has been finalized",
				() => {
			...
			itCompressor("correctly normalizes eagerly allocated final IDs", () => {
			...
			itCompressor(
				"generates correct eager finals when there are outstanding locals after cluster expansion",
				() => {
			...
			itCompressor(
				"generates unique eager finals when multiple outstanding creation ranges during finalizing",
				() => {
	...
	describe("Finalizing", () => {
		itCompressor("prevents attempts to finalize ranges twice", () => {
		...
		itCompressor("prevents attempts to finalize ranges out of order", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
		...
		itCompressor("can finalize ranges into clusters of varying sizes", () => {
			for (let i = 1; i < 5; i++) {
				for (let j = 0; j <= i; j++) {
	...
	describe("Recompression", () => {
		itCompressor("can re-compress a uuid it generated", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
			const uuid = compressor.decompress(id);
		...
		itCompressor("can re-compress uuids from a remote client", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
		...
		itCompressor("will not re-compress uuids it did not originally generate", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			assert.equal(
		...
		itCompressor("can re-compress an eager final ID that is not finalized", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
			compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
	...
	describe("Decompression", () => {
		itCompressor("will not decompress IDs it did not generate", () => {
		...
		itCompressor("can decompress a local ID before and after finalizing", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
			const uuid = compressor.decompress(id);
		...
		itCompressor("can decompress a final ID", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
		...
		itCompressor("can decompress an eagerly generated final ID that is not finalized", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
			compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
			const finalId = compressor.generateCompressedId();
	...
	describe("Normalization", () => {
		itCompressor("can normalize a local ID to op space before finalizing", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
		...
		itCompressor("can normalize a local ID to op space after finalizing", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1);
			const id = compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
			const normalized = compressor.normalizeToOpSpace(id);
		...
		itCompressor("can normalize an eagerly generated final ID", () => {
			const compressor = CompressorFactory.createCompressor(Client.Client1, 5);
			compressor.generateCompressedId();
			compressor.finalizeCreationRange(compressor.takeNextCreationRange());
		...
		itCompressor(
			"cannot normalize a remote ID to session space if it has not been finalized",
			() => {
		...
		itCompressor(
			"can normalize local and final IDs from a remote session to session space",
			() => {
				const compressor1 = CompressorFactory.createCompressor(Client.Client1);
				const compressor2 = CompressorFactory.createCompressor(Client.Client2);
		...
		itCompressor(
			"can normalize a final ID created by the local session but sent in another client's op space",
			() => {
	...
	describe("Serialization", () => {
		...
		itCompressor("correctly deserializes and resumes a session", () => {
			...
			const compressorResumed = CompressorFactory.deserialize(serializedWithSession);
	...
	describe("Collision detection", () => {
		itCompressor(
			"detects when a new cluster is allocated whose UUIDs collide with another cluster",
			() => {
				const compressor1 = CompressorFactory.createCompressor(Client.Client1);
