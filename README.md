...
The set of IDs generated by all Sessions contributing to the same distributed collection (e.g. an individual collaborative document). IDs are not portable between documents.
...
- The ID can be synchronously converted into a full UUID at any time.
...
- **Note:** Session space IDs are only unique within the originating session (for most applications this will mean within each individual client's instnace).
